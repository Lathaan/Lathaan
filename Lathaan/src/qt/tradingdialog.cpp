    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->LTHAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(LTH)"<< "LTH(SIZE)" << "BID(LTH)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(LTH)"  << "LTH(SIZE)" << "TOTAL(LTH)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(LTH)"<<"TOTAL COST(LTH");
...
void tradingDialog::UpdaterFunction(){
    //LTHst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=evo-LTH");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=LTH-LTH&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=LTH-LTH&count=100");
      return Response;
}
...
QString tradingDialog::BuyLTH(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=LTH-LTH&quantity=";
...
QString tradingDialog::SellLTH(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=LTH-LTH";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=LTH";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=LTH-LTH&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->Ask->setText("<b>Ask:</b> <span style='font-weight:bold; font-size:14px; color:Red'>" + str.number(obj["Ask"].toDouble(),'i',8) + "</span> LTH");

    ui->Bid->setText("<b>Bid:</b> <span style='font-weight:bold; font-size:14px; color:Green;'>" + str.number(obj["Bid"].toDouble(),'i',8) + "</span> LTH");

    ui->volumet->setText("<b>LTH Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> LTH");

    ui->volumeevo->setText("<b>LTH Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["BaseVolume"].toDouble(),'i',8) + "</span> LTH");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        ui->LTHSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(LTHSupply,'i',8) + "</span><b> LTH</b>");
        ui->EvoSupply->setText("<span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(EvoSupply,'i',8) + "</span><b> LTH</b>");
        ...
        ui->LTHDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(LTHDemand,'i',8) + "</span><b> LTH</b>");
        ui->EvoDemand->setText("<span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(EvoDemand,'i',8) + "</span><b> LTH</b>");
    ...
    switch (index){
                case 0:    //buy tab is active
                           ...
                           if(Response.size() > 0 && Response != "Error"){
                               ...
                               QString balance = GetBalance("LTH");
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       ui->LTHAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("LTH");
                         if(Response.size() > 0 && Response != "Error"){
                          DisplayBalance(*ui->LathaanBalanceLabel,*ui->LathaanAvailableLabel,*ui->LathaanPendingLabel, QString::fromUtf8("LTH"),Response);
                         }

                         Response = GetBalance("LTH");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->LTHBalanceLabel,*ui->LTHAvailableLabel,*ui->LTHPendingLabel, QString::fromUtf8("LTH"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputLTH->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of LTH that can be gained from selling LTH available balance
    QString responseA = GetBalance("LTH");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableLTH = ResultObject["Available"].toDouble();

    ui->UnitsInputLTH->setText(str.number(AvailableLTH,'i',8));
}

void tradingDialog::on_Buy_Max_Amount_clicked()
{
    //calculate amount of currency than can be brought with the LTH balance available
    QString responseA = GetBalance("LTH");
    ...
    double Result = (AvailableLTH / CurrentASK);
...
void tradingDialog::on_BuyLTH_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyLTH(Order,Quantity,Rate);
...
void tradingDialog::on_SellLTHBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputLTH->text().toDouble();
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellLTH(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputLTH_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}

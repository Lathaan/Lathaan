...
{
    QString ret = QString("Lathaan:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non LTH unit input in lnReqAmount, we generate the URI with LTH as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(LathaanUnits::format(LathaanUnits::LTH, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

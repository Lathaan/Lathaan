...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /c/deps/lathaan-master/win32-build/src/lathaand.exe $OUT_DIR/lathaand.exe
  cp /c/deps/lathaan-master/win32-build/src/test/test_lathaan.exe $OUT_DIR/test_lathaan.exe
  cp /c/deps/lathaan-master/win32-build/src/qt/lathaand-qt.exe $OUT_DIR/lathaan-qt.exe
  # Linux:
  cp /c/deps/lathaan-master/linux-build/src/lathaand $OUT_DIR/lathaand
  cp /c/deps/lathaan-master/linux-build/src/test/test_lathaan $OUT_DIR/test_lathaan
  cp /c/deps/lathaan-master/linux-build/src/qt/lathaand-qt $OUT_DIR/lathaan-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /c/deps/lathaan-master/linux-build
make check

# Run RPC integration test on Linux:
/c/deps/lathaan-master/qa/rpc-tests/wallet.sh /c/deps/lathaan-master/linux-build/src
/c/deps/lathaan-master/qa/rpc-tests/listtransactions.py --srcdir /c/deps/lathaan-master/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /c/deps/lathaan-master/win32-build
  make check
fi
